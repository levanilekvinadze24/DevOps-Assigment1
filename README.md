...
### Live Application
`https://dev-ops-assigment1.vercel.app`
...
### Trigger Conditions
- Push to `main`: CI runs ✅, CD runs ✅ (only if CI passes)
- Pull request to `main`: CI runs ✅, CD runs ❌
- Push to other branches: (not used here; optional)

### Project Structure
- `.github/workflows/main.yml` — CI/CD pipeline
- `api/index.py` — FastAPI app (Vercel Python serverless entry)
- `tests/test_health.py` — PyTest suite
- `requirements.txt` — Python dependencies
- `vercel.json` — Vercel build/routes config

### API Endpoint
- `GET /` → returns `{ "status": "ok", "service": "DevOps-Assigment1" }`

### Update Strategy: Blue-Green (Simulated)
**Strategy chosen:** Blue-Green (simulated with PR gate)

**How it’s applied safely:**
- `main` acts as **Production** (auto-deployed only after tests pass).
- Pull Requests act as the **safe validation gate** (tests must pass before merging).
- (Optional) A separate `staging` branch + separate Vercel project can be used for a true Blue-Green split, but free-tier can be kept simple with PR gating.

### Rollback Guide (Vercel)
**Option A — Rollback via Vercel Dashboard (Fastest)**
1. Open Vercel dashboard → your project
2. Go to **Deployments**
3. Pick the last stable deployment
4. Click **Promote to Production**

**Option B — Git Revert + Push (Auditable)**
1. Revert the bad commit in Git and push to `main`
2. CI runs tests again
3. If tests pass, CD deploys the reverted stable version automatically
